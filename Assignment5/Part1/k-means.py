...
class Cluster:
    ...
    def update(self, points):
        '''
        1.Recalculates and stores the new centroid.
		2.Returns the distance between the previous centroid and the new one       
        '''
...
def validation_sse(list_of_clusters):
	...
	for i in list_of_clusters:
		...
		for j in i:#j refers to individual point in i
			...
			term1 = (getDistance(list_of_centroids[itr].centroid,j)**2)
...
for j in split_data:
	...
	if(i!=0):
		...
		list_of_coordinate_points.append(point1) #label,x,y,cluster
...
if(num_clusters>len(list_of_coordinate_points)):
	print "The number of coordinate points is ",len(list_of_coordinate_points)
	print "Please input k value lesser than number of coordinate points"
	exit(0)
list_of_clusters = kmeans_clustering(list_of_coordinate_points, num_clusters, opt_cutoff)
